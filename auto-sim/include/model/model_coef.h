...
public:
    ...
    /// Load a single model (e.g. severity) from a coef file under the given name.
    void read_model(const std::string& path, const std::string& model_name);

    /// Severity linear predictor: calc() + the recovery_lag_bkt*platform interaction
    /// (dumped under var 'platform' as composite keys "<platform>:recovery_lag_bkt<bkt>").
    double severity_calc(const LoanDict& loan, const std::string& model_name) const;
