...
``rate``, ``dti``), and produces a dict of breakdown DataFrames
...
def _bkt_order_key(label) -> int:
    """Order any leading-numeric bucket label: term '<=60'->60, fico '580-620'->580,
    rate '25%+'->25.  Non-numeric labels sort last."""
    digits = "".join(c if c.isdigit() else " " for c in str(label)).split()
...
def compute_deal_input_stats(input_df: pd.DataFrame, *, balance_col: str = "balance") -> dict:
    ...
    Returns a dict with:
      ...
      - ``by_term_grade``, ``by_term_fico``, ``by_term_rate`` :
        DataFrames crossed with term, with TOTAL rows
    """
    if input_df.empty:
        return {}
    ...
    if "term" in df.columns:
        overview["w_term"] = wavg(df, "term", bal)  # weighted avg over the raw numeric term

    out: dict = {"overview": overview}

    # Group over the model's native term bucket when present (raw term is
    # near-continuous for auto); relabel to "term" for uniform rendering.
    term_col = "term_bkt" if "term_bkt" in df.columns else ("term" if "term" in df.columns else None)
    if term_col is None:
        return out

    def _by(dims: list[str]) -> pd.DataFrame:
        g = _group_by(df, [term_col] + dims, bal, wavg_cols)
        return g.rename(columns={term_col: "term"}) if term_col != "term" else g

    key = lambda s: s.map(_bkt_order_key)
    out["by_term"] = _by([]).sort_values("term", key=key)

    # FICO / rate cross-tabs: prefer the tape's native bucket, else re-bucket.
    for col, bins, labels in _BUCKET_DEFS:
        bkt = f"{col}_bkt"
        if bkt not in df.columns:
            if col not in df.columns:
                continue
            cut_bins = list(bins) + ([np.inf] if len(bins) == len(labels) else [])
            df[bkt] = pd.cut(df[col], bins=cut_bins, labels=labels, right=False)
        grouped = _by([bkt]).sort_values(["term", bkt], key=key)
