...
Reads the ``Portfolio`` and ``Metrics_Portfolio`` sheets into a dict of scalar
KPIs plus derived per-period series (``pool_factor``, ``cum_interest``), and
enriches ``Metrics_Portfolio`` with balance-weighted CTD1/CTP from
``Metrics_Grouped`` (used on the curves page DQ tab).
"""
...
# ---------------------------------------------------------------------------
# Chart x-horizon — one rule shared by every page: cut a curve at the last
# period with >= POOL_CUTOFF of the original pool outstanding. Keeps the
# horizon consistent across pages instead of per-chart thresholds.
# ---------------------------------------------------------------------------
POOL_CUTOFF = 0.002  # 0.20% of the original pool


def pool_horizon(begin_bal, period, cutoff: float = POOL_CUTOFF) -> int | None:
    """Last `period` where `begin_bal` >= cutoff × initial pool.

    Falls back to the max period if the pool never drops below the cutoff, or
    None if there's no data."""
    bb = pd.Series(list(begin_bal)).reset_index(drop=True)
    pe = pd.Series(list(period)).reset_index(drop=True)
    if bb.empty:
        return None
    init = bb.iloc[0]
    if not init:
        return int(pe.max())
    keep = pe[bb >= cutoff * init]
    return int(keep.max()) if len(keep) else int(pe.max())
