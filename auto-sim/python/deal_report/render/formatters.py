...
def fmt_cell(val, spec: str | None) -> str:
    ...
    if spec.startswith("$"):
        prefix, spec = "$", spec[1:]
    # ".2%" is native Python (renders "5.00%"), but d3-style ".2f%"/".0f%" mean
    # "format the raw number, then append %" — Python can't parse those, so strip
    # the % and re-append. A type char before % marks the d3 form.
