...
- Loan input:  ``input/deals/<deal>/<deal>.csv`` (or first CSV in folder),
               falling back to the prepped tape ``loans_prepped.json`` that
               ``auto_tape.py`` writes (auto has no raw per-deal CSV).
"""
...
def load_deal_input(deal: str, *, input_dir: Path | None = None) -> pd.DataFrame:
    """Load the deal's loan tape for pool-composition stats.

    Prefers a raw CSV; falls back to the prepped ``loans_prepped.json`` tape
    (auto's convention). Returns an empty DataFrame when neither exists.
    """
    csv_path = resolve_input_csv(deal, input_dir=input_dir)
    if csv_path is not None:
        return pd.read_csv(csv_path)
    base = Path(input_dir) if input_dir else model_root() / "input" / "deals"
    prepped = base / deal / "loans_prepped.json"
    if prepped.is_file():
        return pd.read_json(prepped)
    return pd.DataFrame()
