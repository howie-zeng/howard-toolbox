...
from simengine.data_prep import (
    ModelLink,
    load_all_coef,
    build_all_models,
    build_model_link,
...
def generate_model_report_html(
    ...
    severity_link = _load_severity_link(coef_dir, config)
    if output_path is None:
        output_path = os.path.join(coef_dir, "model_report.html")
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
    coef_version = os.path.basename(coef_dir)
    html = _build_html(status_to_roll, all_models, coef_version, severity_link)
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"  Model report: {output_path}")


def _load_severity_link(coef_dir: str, config: Dict[str, Any]) -> "ModelLink | None":
    """Load the severity (recovery-rate) GAM as a ModelLink, or None if not
    configured / file absent. Same coef format as the transition models, so it
    reuses _build_cat_table + _build_smooth_charts."""
    sev_coef = (config or {}).get("severity_coef")
    if not sev_coef:
        return None
    sev_path = os.path.join(coef_dir, sev_coef)
    if not os.path.isfile(sev_path):
        return None
    return build_model_link(read_coef_file(sev_path))


def _build_html(status_to_roll, all_models, coef_version, severity_link=None):
    from_statuses = list(status_to_roll.keys())
    ...
    # Severity model — one top tab, single sub-page (no sub-nav). The JS
    # lazy-renders charts off the .sub-page.active wrapper when the tab opens.
    if severity_link is not None:
        page_id = "page-severity"
        top_tabs_html.append(f'<a href="#" data-page="{page_id}">Severity</a>')
        cat_html = _build_cat_table(severity_link, "Recovery Rate")
        smooth_html, specs = _build_smooth_charts(
            severity_link, "Recovery Rate", "sev", "rr")
        all_specs.update(specs)
        pages_html.append(
            f'<div id="{page_id}" class="report-page"><div class="content">\n'
            f'<div class="sub-page active">\n{cat_html}\n{smooth_html}\n</div>\n'
            f'</div></div>')
...
.modal-hint {{ font-size: 11px; color: #6c757d; margin-top: 8px; text-align: center; }}
  ...
  <!-- Pinned exact trio (unpinned majors drift apart on the CDN and blank the
       charts). Keep in sync with deal_report/assets.py::VEGA_CDN. -->
