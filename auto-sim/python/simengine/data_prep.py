...
# Only renames if old exists and new doesn't (safe for either format).
# =====================================================================
DV01_RENAME_MAP = {
    "dv01_id":                "loan_id",
    "loan_term_orig":         "term",
...
# field_name -> (value_map, unmapped_default, none_default).
# raw None -> none_default (None means stays None, v_flag handles it);
# raw present but unmapped -> unmapped_default.
FIELD_VALUE_MAPS = {
...
# c_* fields are auto-copied from their base field; Smooth1D.eval clamps to
# knot bounds. Missing smooth fields with v_* flags auto-default to 0.
# =====================================================================
DERIVED_FIELD_NAMES = {
    "int_rate", "age", "age_pct",
    "pmt_day", "days_to_month_end",
    "lending_environment",
...
def derive_initial_fields(loan: Dict, c_fields: set = frozenset(),
    ...
    """Compute derived fields once per loan at prep. Delegates to VarRegistry.

    c_fields: c_* field names from coef files (auto-copied from base).
    ctx: lookup tables (cpi_lookup, fico_coupon_lookup, ...).
    """
    _get_registry().derive_initial(loan, ctx=ctx, c_fields=c_fields)


# =====================================================================
# TIME-VARYING FIELDS — delegate to VarRegistry (register_vars.py)
# =====================================================================

_DEFAULT_REGISTRY: Optional[VarRegistry] = None  # lazy singleton
...
def step_transition_fields(loan: Dict, from_status: str, to_status: str) -> None:
    """Update PATH_DEPENDENT vars from the drawn from->to transition."""
    _get_registry().step_transition(loan, from_status, to_status)
...
# =====================================================================
RECOMMENDED_FIELDS = ["term_bkt", "orig_bal"]
...
# ---------------------------------------------------------------------------
# Coef file reader  (mirrors model_coef.read from the Python reference)
# ---------------------------------------------------------------------------
...
def classify_model_terms(models: Dict[str, ModelLink], dynamic_vars: Set[str]) -> None:
    """Split each model's terms into static (never change) vs dynamic (depend on a
    time-varying/macro var), stored as _static_*/_dynamic_* lists on the ModelLink
    for build_static_cache / calc_dynamic.
    """
...
def load_dial(path: str, all_status: List[str], n_per: int) -> Dict:
    ...
    # Segment cols = anything between Status and the first status-name column
    ...
    with open(path, "r") as f:
        ...
        for row in reader:
            ...
            # Segment key updates only on header rows (from_val non-empty)
            ...
            # Period counter is per (from_status, segment)
...
def _apply_value_maps(loan: Dict, field_maps: Dict) -> None:
    ...
    for field_name, map_tuple in field_maps.items():
        ...
        if field_name not in loan:
            loan[field_name] = none_default  # absent key treated as None
            continue
        raw = loan[field_name]
...
def extract_model_fields(coef_dir: str, from_statuses: List[str]) -> Dict[str, Any]:
    ...
    # Track which models use a smooth with vs without a v_flag (for consistency warnings)
    smooth_with_flag: Dict[str, set] = {}    # {var_name1: {models}}
    smooth_without_flag: Dict[str, set] = {} # {var_name1: {models}}
    for fs in from_statuses:
        path = os.path.join(coef_dir, f"from{fs}.txt")
        if not os.path.isfile(path):
            continue
        with open(path) as f:
            ...
            for row in reader:
                ...
                if vn2:
                    ...
                    # No var_name2: numeric var_val1 => smooth without a v_flag
    ...
    # Warn when a field has a v_flag in some models but not others
    ...
    # Also: bare field X used without flag while c_X has one
    ...
    # Fields unflagged in some models: if data is missing, imputed 0 is treated as real
...
def _extend_cpi_forward(lookup: Dict[str, float], n_months: int = 120) -> None:
    """Project CPI forward n_months at the trailing 12-month growth rate."""
    if not lookup:
        return
...
def _trace_missing_root(field_name: str, deps: Dict[str, List[str]],
    ...
    while current in deps:
        ...
        if not missing_src:
            return None  # sources present — derivation should have worked
...
def prepare_loans(
    ...
    v_flags = model_info["v_flags"]           # {v_flag: paired var_name1}
    model_fields = model_info["all_fields"]   # var_name1 + var_name2 (excl v_*)
    unflagged_smooth = model_info.get("unflagged_smooth", {})

    # -- Model summary --
    if cat_levels or model_fields:
        # Smooth = model fields that are not categorical
        ...
        if cat_field_names:
            ...
            for cf in sorted(cat_field_names):
                ...
                # Reference level has no coef row (absorbed into intercept); any
                # observed value not in `levels` is treated as reference.
    ...
    # Smooth fields with a v_* flag default to 0 when missing (v_flag=0 zeroes the
    # contribution, so the placeholder value is irrelevant).
    smooth_defaults = {}
    for flag_name, smooth_var in v_flags.items():
        smooth_defaults[smooth_var] = 0
        # v_X -> c_X: also default the bare base X (some models use it directly)
    ...
    # Fields that are derived/macro/auto (not expected from raw input)
    ...
    lookup_names = {"cpi_inflator_36", "cpi_inflator_12", "rate_incentive_ALL"}
    ...
    for i, loan in enumerate(loans):
        ...
        # 3. Derived fields + macro lookups via registry init_fns. month /
        #    days_to_month_end are the single source of truth (report-anchored).
        ...
        # 8. Missing smooth defaults (AFTER v_flags so flags saw the None first)
        ...
        # 9. Field-level validation (first loan only)
        if i == 0:
            # REQUIRED: model + cashflow fields
            missing_required = []
...
def _bucket_of_lag(L: int) -> str:
    """Map an integer recovery lag (months) to the GAM's recovery_lag_bkt level."""
    if L <= 0:  return "0"
    if L == 1:  return "1"
    if L <= 3:  return "2-3"
    if L <= 6:  return "4-6"
...
def _load_lag_dist(path: str):
    """recovery_lag_dist.tsv (platform<tab>lag_bucket<tab>prob) -> ({platform: [(label, cum_prob)...]},
    {label: (lag_months|None, recovery_lag_bkt|None)}).  label 'never' -> no recovery."""
    rows: Dict[str, List] = {}
    with open(path, newline="") as f:
        for r in csv.DictReader(f, delimiter="\t"):
            rows.setdefault(r["platform"], []).append((r["lag_bucket"], float(r["prob"])))
    cdf, label_map = {}, {}
    for plat, lst in rows.items():
        c, out = 0.0, []
        for lab, p in lst:
            c += p; out.append((lab, c))
        cdf[plat] = out
        for lab, _ in lst:
            if lab in label_map:
                continue
            if lab == "never":   label_map[lab] = (None, None)
            elif lab == "13+":   label_map[lab] = (13, "13+")
            else:
                try:             label_map[lab] = (int(lab), _bucket_of_lag(int(lab)))
                except ValueError: label_map[lab] = (None, None)
    return cdf, label_map


def _load_manheim(path: str) -> Dict[int, float]:
    """MANHEIM.csv (DATE,MANHEIM) -> {year*12+month: value}, gap-filled forward."""
    ...
    with open(path, newline="") as f:
        for r in csv.DictReader(f):
            ...
            if dt is not None:
                pts[dt.year * 12 + dt.month] = float(r["MANHEIM"])
    if not pts:
        return {}
    out, last = {}, None
    for ym in range(min(pts), max(pts) + 1):
        if ym in pts: last = pts[ym]
        out[ym] = last
    return out
...
def init_data_manager(
    ...
    dm.severity_model = None
    dm.lag_cdf = {}
    dm.lag_label_map = {}
    dm.manheim = {}
    sev_coef = config.get("severity_coef") if config else None
    lag_path = config.get("lag_dist_path") if config else None
    if sev_coef and lag_path:
        sev_path = os.path.join(coef_dir, sev_coef)
        if os.path.isfile(sev_path):
            dm.severity_model = build_model_link(read_coef_file(sev_path))
        if os.path.isfile(lag_path):
            dm.lag_cdf, dm.lag_label_map = _load_lag_dist(lag_path)
        mh_path = config.get("manheim_path", os.path.join(input_dir, "macro", "MANHEIM.csv"))
        if os.path.isfile(mh_path):
            dm.manheim = _load_manheim(mh_path)
        print(f"  Severity: model={'loaded' if dm.severity_model else 'MISSING'}, "
              f"lag platforms={len(dm.lag_cdf)}, manheim months={len(dm.manheim)}")
...
def dump_gam_models(
    ...
    r_script = config.get("r_script", "tools/dump/dump_gam_to_coef.R")
