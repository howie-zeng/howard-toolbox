...
struct Config {
    ...
    std::string severity_coef;     // filename under coef_dir (e.g. "severity.txt")
    std::string lag_dist_path;     // recovery-lag distribution csv
    std::string manheim_path;      // Manheim index csv
...
Config parse_config(const std::string& path) {
    ...
    while (true) {
        ...
        } else if (key == "severity_coef") {
            cfg.severity_coef = p.parse_string();
        } else if (key == "lag_dist_path") {
            cfg.lag_dist_path = p.parse_string();
        } else if (key == "manheim_path") {
            cfg.manheim_path = p.parse_string();
...
struct MetricRow {
    int period;
    double cpr, cdr, cgl, cnl;
    double begin_bal, pif_bal, liq_bal, loss, cum_loss, cum_recov;
...
static std::vector<MetricRow> compute_metrics(
    ...
    int i_recov   = rrm::CF_DICT.at("recov");

    double cum_loss = 0.0;
    double cum_recov = 0.0;
    ...
    for (int p = 0; p < static_cast<int>(cf.size()); ++p) {
        ...
        double cnl = (orig_bal > 0) ? (cum_loss - cum_recov) / orig_bal : 0.0;

        MetricRow mr;
        mr.period = p + 1;
        mr.cpr = cpr; mr.cdr = cdr; mr.cgl = cgl; mr.cnl = cnl;
        mr.begin_bal = bb; mr.pif_bal = pif_bal; mr.liq_bal = liq_bal;
        mr.loss = loss; mr.cum_loss = cum_loss; mr.cum_recov = cum_recov;
...
static void write_metrics_portfolio_csv(
    ...
    f << std::fixed;
    f << "period,cpr,cdr,cgl,cnl,begin_bal,pif_bal,liq_bal,loss,cum_loss,cum_recov,sch_prin\n";
...
static void write_metrics_grouped_csv(
    ...
    for (const auto& key : keys) {
        ...
        // Age/period alignment: row for loan_age N carries financials from
        // metrics[N-1] (transition at N-1, observed at N) and probs[N] (predicts N+1).
        // cf_array[i] == metrics[i], labeled period i+1, belongs to loan_age i+1.
        ...
        for (int age = 0; age < max_age; ++age) {
            bool has_probs = pw_ptr && pb_ptr
                ...
                && (*pb_ptr)[age] > 0.0;
            // Financials for this age come from metrics[age-1]
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        dm.init(cfg.input_dir, cfg.status_to_roll, cfg.dial_name, cfg.n_per, cfg.coef_version, cfg.macro_vars,
                cfg.severity_coef, cfg.lag_dist_path, cfg.manheim_path);
