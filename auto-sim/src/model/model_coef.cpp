...
void ModelCoef::read_model(const std::string& path, const std::string& model_name) {
    auto rows = io::read_coef_file(path);
    build_model_auto(model_name, rows);
}
...
// Severity LP = generic calc (main effects + smooths, incl. platform + recovery_lag_bkt mains)
// + the recovery_lag_bkt*platform interaction, dumped under var 'platform' as composite-key
// levels "<platform>:recovery_lag_bkt<bkt>" (generic calc misses it — it looks up one key per var).
double ModelCoef::severity_calc(const LoanDict& loan, const std::string& model_name) const {
    double lp = calc(loan, model_name);
    auto ml_it = model_link_.find(model_name);
    if (ml_it == model_link_.end()) return lp;
    auto t_it = ml_it->second.terms_1d.find("platform");
    if (t_it != ml_it->second.terms_1d.end()) {
        if (auto* info = std::get_if<Lookup1dInfo>(&t_it->second)) {
            std::string key = loan_str(loan, "platform") + ":recovery_lag_bkt"
                            + loan_str(loan, "recovery_lag_bkt");
            for (const auto& [level, coef] : info->levels) {
                if (level == key) { lp += coef; break; }
            }
        }
    }
    return lp;
