...
void init_time_varying(LoanDict& loan) {
    ...
    // Initialize time-varying fields at period -1 (pre-sim state). age fraction is
    // over the ORIGINAL term (loan's full life), not the remaining term.
    int term = get_int(loan, "orig_term");
